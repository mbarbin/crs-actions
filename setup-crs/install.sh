...
REPO="mbarbin/crs"
VERSION="${CRS_VERSION}"
if [ -z "${CRS_DIGEST:-}" ]; then
  echo "Error: CRS_DIGEST environment variable must be set to an expected digest (e.g., sha256:abc123...)." >&2
  exit 1
fi
...
gh attestation verify "${bin_path}" --owner mbarbin --signer-repo "${REPO}"
echo "::endgroup::"
